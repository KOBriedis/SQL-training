...
describe("Queries Across Tables", () => {
  ...
  it(
    ...
    async done => {
      const query =
        `SELECT ROUND(SUM(movies.budget_adjusted),2) AS 'total_budget', directors.full_name AS 'director'
      FROM movies
  ...
  it(
    ...
    async done => {
      const query =
        `SELECT COUNT(*) AS 'count', keywords.keyword
      FROM movies
      JOIN movie_keywords
  ...
  it(
    ...
    async done => {
      const query =
        `SELECT COUNT(*) AS 'five_stars_count', genre
      FROM genres
      JOIN movie_genres
  ...
  it(
    ...
    async done => {
      const query =
        `SELECT ROUND(AVG(rating),2) AS 'avg_rating', genre
      FROM genres
      JOIN movie_genres
      ON genres.id = movie_genres.genre_id 
      JOIN movie_ratings

...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query =
        `SELECT ROUND(SUM(budget_adjusted), 2) AS 'total_budget', ROUND(SUM(revenue_adjusted), 1) AS 'total_revenue'
       FROM movies`;
  ...
  it(
    ...
    async done => {
      ...
      AND release_date >= date('2009-01-01')`;
      const result = await db.selectSingleRow(query);
  ...
  it(
    ...
    async done => {
      const query =
        `SELECT COUNT(*) AS 'count', SUBSTR(release_date, 1, 4) AS 'year'
      FROM movies
      GROUP BY year
      ORDER BY year DESC`;
      const result = await db.selectMultipleRows(query);
  ...
  it(
    ...
    async done => {
      const query =
        `SELECT COUNT(*) AS 'count', SUBSTR(time_created, 6, 2) AS 'month'
      FROM movie_ratings
      GROUP BY month
      ORDER BY count DESC`;
      const result = await db.selectMultipleRows(query);
